...
def convert_to_png_image(image, drange_in=[0,1], drange_out=[0,255]):
    ...
    image = adjust_dynamic_range(image, drange_in, drange_out)
    image = np.rint(image).clip(*drange_out).astype(np.uint8)   
    
    format = 'RGB' if image.ndim == 3 else 'L'
    #return PIL.Image.fromarray(image, format)
    return png.from_array(image.copy(), mode = format + ';8')

def save_image(image, filename, drange=[0,1], quality=95):
    img = convert_to_png_image(image, drange)
    if '.jpg' in filename:
        img.save(filename,"JPEG", quality=quality, optimize=True)
    else:
        img.save(filename)

def save_image_grid(images, filename, drange_in=[0,1], drange_out=[0,255], grid_size=None):
    convert_to_png_image(create_image_grid(images, grid_size), drange_in, drange_out).save(filename)
