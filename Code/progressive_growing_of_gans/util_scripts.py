...
def generate_fake_images(run_id, snapshot=None, grid_size=[1,1], num_pngs=1, drange_in = [0,1], drange_out = [0,255], image_shrink=1, png_prefix=None, random_seed=1000, minibatch_size=8):
    ...
    for png_idx in range(num_pngs):
        ...
        images = Gs.run(latents, labels, minibatch_size=minibatch_size, num_gpus=config.num_gpus)
        misc.save_image_grid(images, os.path.join(result_subdir, '%s%06d.png' % (png_prefix, png_idx)), drange_in, drange_out, grid_size)
...
def evaluate_metrics(run_id, log, metrics, num_images, real_passes, minibatch_size=None):
    ...
    for title, mode in [('Reals', 'reals'), ('Reals2', 'fakes')][:real_passes]:
        ...
        for begin in range(0, num_images, minibatch_size):
            ...
            #images = misc.adjust_dynamic_range(images, [0,6], [0, 255])
    ...
    for snapshot_idx, snapshot_pkl in enumerate(reversed(snapshot_pkls)):
        ...
        with tf.Graph().as_default(), tfutil.create_session(config.tf_config).as_default():
            ...
            for begin in range(0, num_images, minibatch_size):
                ...
                images = Gs.run(latents, labels[begin:end], num_gpus=config.num_gpus)
                images = misc.adjust_dynamic_range(images, [-1,1], [0, 6])
                images = np.rint(images).clip(0,6).astype(np.uint8)
