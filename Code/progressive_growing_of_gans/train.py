...
def train_progressive_gan(
    ...
    resume_run_id           = 47,         # Run ID or network pkl to resume training from, None = start from scratch.
    resume_snapshot         = None,         # Snapshot index to resume training from, None = autodetect.
    resume_kimg             = 8240.1,          # Assumed training progress at the beginning. Affects reporting and training schedule.
    resume_time             = 242100):         # Assumed wallclock time at the beginning. Affects reporting.
    ...
    misc.save_image_grid(grid_reals, os.path.join(result_subdir, 'reals.png'), drange_in=training_set.dynamic_range, drange_out = [0,255], grid_size=grid_size)
    misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % 0), drange_in=drange_net, drange_out = [0,255], grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=sched.minibatch//config.num_gpus)
                misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % (cur_nimg // 1000)), drange_in=drange_net, drange_out = [0,255], grid_size=grid_size)
