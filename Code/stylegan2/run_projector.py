...
def project_image(proj, targets, png_prefix, num_snapshots):
    ...
    misc.save_image_grid(targets, png_prefix + 'target.png', drange_in=[-1,1], drange_out =[0,255])
    proj.start(targets)
    while proj.get_cur_step() < proj.num_steps:
        print('\r%d / %d ... ' % (proj.get_cur_step(), proj.num_steps), end='', flush=True)
        proj.step()
        if proj.get_cur_step() in snapshot_steps:
            misc.save_image_grid(proj.get_images(), png_prefix + 'step%04d.png' % proj.get_cur_step(), drange_in=[-1,1], drange_out = [0,255])
